...
export interface ActiveTrain {
  ...
  position: [number, number];
  bearing: number;
  delay: number; // ← thêm để UI hiển thị nếu cần
}

export type DelayLookup = (railway: string, trainNumber: string) => number;
...
export function getActiveTrains(
  ...
  stationsById: Map<string, Station>,
  pathIndex: RailwayPathIndex,
  getDelay?: DelayLookup,
): ActiveTrain[] {
  const active: ActiveTrain[] = [];

  for (const tt of timetables) {
    const delay = getDelay?.(tt.railway, tt.trainNumber) ?? 0;

    // "Schedule time" mà tàu này thực tế đang ở. Delay dương = tàu chậm
    // → vị trí của nó tương ứng với schedule trước đó (currentSeconds - delay).
    const scheduledNow = currentSeconds - delay;

    // Thử match cả ở "ngày này" và "ngày trước" (cho tàu chạy qua nửa đêm
    // được encode dạng 25:30 = 91800s).
    const tryRender =
      tryActiveAt(tt, scheduledNow, stationsById, pathIndex, delay) ??
      tryActiveAt(
        tt,
        scheduledNow + SECONDS_PER_DAY,
        stationsById,
        pathIndex,
        delay,
      );

    if (tryRender) active.push(tryRender);
  }
